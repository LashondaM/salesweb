...
public class CustomersController {
	...
	public ResponseEntity<Iterable<Customer>> GetAll() {
		var customers = custRepo.findAll(); // in c# we would have to use Async, await, etc -- Java runs Asynchronously automatically
		return new ResponseEntity<Iterable<Customer>>(customers, HttpStatus.OK); // status is a 200 OK
	}
	...
	public ResponseEntity<Customer> GetById(@PathVariable int id) {
		...
		return new ResponseEntity<Customer>(customer.get(), HttpStatus.OK); // get() returns the customer inside the optional
	}
